...
Title: EU27 & UK gas supply-transmission-consumption structures with driving
       factors of consumption change
Version: 2.0
...
Changes made: 
(1) Raw CSV data transformed into an optimized PostgreSQL database with modified
    table and column names, 
    custom indexes and constraints, 
    analytical views, 
    and market analysis queries.
(2) Original data (EUGasSC.csv & EUGasNet.csv) was accessed and transformed
    into PostgreSQL schema in December 2024.
(3) Attribution is placed in this file (import.sql) as it handles the direct
    importation from the source CSV files.
...
CREATE TEMPORARY TABLE IF NOT EXISTS "EUGasSC_staging" (
    ...
    "PRO_from_storage" BIGINT DEFAULT NULL, -- Supply from stored EU production.
...
\copy "EUGasSC_staging" FROM '../../../EUGasSC.csv' WITH (
...
-- Note: Share columns ('_share') represent source composition ratios of transmitted gas
...
\copy "EUGasNet_staging" FROM '../../../EUGasNet.csv' WITH (
    FORMAT csv, HEADER true
);