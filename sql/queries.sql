...
-- Find trading relationships that transformed from median pre-invasion Russian dependency to lower quartile post-invasion:
/* Using P50 (0.5666) from pre-invasion and P25 (0.3323) from post-invasion to identify relationships
that showed significant but not complete reduction in Russian gas dependency
*/
SELECT DISTINCT
    "export_country_code",
    "import_country_code"
...
/* Analysis focused on relationships that shifted from median (P50) pre-invasion Russian dependency
to below lower quartile (P25) post-invasion, identifying significant strategic adaptations.

P50 pre-invasion (0.5666): Shows relationships with above-median Russian gas dependency
P25 post-invasion (0.3323): Captures relationships that reduced dependency below lower quartile
*/
SELECT
    "export_country_code",
    "import_country_code",
...
WHERE ("export_country_code", "import_country_code") IN (
    ...
    FROM "documented_routes"
    WHERE "date" < '2022-02-24'
        AND "RU_share" > 0.5666  -- P50 pre-invasion
    EXCEPT
    SELECT "export_country_code", "import_country_code"
    FROM "documented_routes"
    WHERE "date" >= '2022-02-24'
        AND "RU_share" > 0.3323  -- P25 post-invasion
)
...
-- Analyze Romania's export relationships pre and post invasion:
SELECT
    CASE
        WHEN "date" < '2022-02-24' THEN 'Pre-invasion'
        ELSE 'Post-invasion'
    END AS "period",
    CONCAT('RO ----> ', "import_country_code") AS "route",
    AVG("RU_share")::NUMERIC(5,4) AS "avg_RU_share",
    AVG("LNG_share")::NUMERIC(5,4) AS "avg_LNG_share",
    AVG("PRO_share")::NUMERIC(5,4) AS "avg_PRO_share",
    AVG("NO_share")::NUMERIC(5,4) AS "avg_NO_share",
    COUNT(*) AS "n_transmissions"
FROM "documented_routes"
WHERE "export_country_code" = 'RO'
AND "import_country_code" IN ('BG', 'HU')
GROUP BY
    CASE
        WHEN "date" < '2022-02-24' THEN 'Pre-invasion'
        ELSE 'Post-invasion'
